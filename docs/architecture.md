...
Craft Agents bundles OAuth callback servers, a Slack/Teams/Lark messaging gateway, a WhatsApp worker backed by Baileys, a session MCP server, a bridge MCP server, a `gen_image` provider, an `apps/viewer` Electron app for public sharing, and Sources with per-source MCP bridge processes. MkAgent does not ship any of these; see [`comparison-with-craft.md`](./comparison-with-craft.md) for an evidence-backed side-by-side and the resulting installer-size delta.
