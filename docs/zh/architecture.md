...
Craft Agents 自带 OAuth callback server、Slack/Teams/Lark messaging gateway、基于 Baileys 的 WhatsApp worker、session MCP server、bridge MCP server、`gen_image` provider、独立 `apps/viewer` Electron 应用以及每个 source 的 MCP bridge 进程。MkAgent 一个都不打包,具体证据与安装包体积差异见 [`comparison-with-craft.md`](./comparison-with-craft.md)。
