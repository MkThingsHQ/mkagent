...
import { sanitizeForTitle } from '@mkagent/server-core/domain'
...
describe('sanitizeForTitle', () => {
  ...
  describe('whitespace normalization', () => {
    ...
    it('handles whitespace left after stripping mentions', () => {
      expect(sanitizeForTitle('[skill:commit]   [file:/tmp/input.md]   do work')).toBe('do work')
    })
  })

  describe('multiple mentions in one title', () => {
    it('strips mixed mention types', () => {
      expect(sanitizeForTitle('[skill:commit] [folder:/a] [file:/a/b.ts] fix everything')).toBe('fix everything')
    })

    it('strips mentions interspersed with text', () => {
      expect(sanitizeForTitle('use [skill:commit] and [folder:/repo] to fix [file:/a.ts]')).toBe('use and to fix')
    })
  })

  describe('edge cases', () => {
    it('returns empty string when only mentions present', () => {
      expect(sanitizeForTitle('[skill:commit] [file:/tmp/input.md]')).toBe('')
    })
