...
> **CLI-first workflow (recommended):** Use `mkagent label ...` commands instead of editing JSON directly.
> - `mkagent label --help`
> - Canonical command reference: [craft-cli.md](./craft-cli.md)

## Storage Locations

- Config: `~/.mkagent/workspaces/{id}/labels/config.json`
...
Prefer `mkagent` commands:

```bash
mkagent label create --name "Bug" --color "destructive"
mkagent label create --name "Priority" --color "accent" --value-type number
mkagent label create --name "Due Date" --color "info" --value-type date
mkagent label create --name "Docs" --color "info" --value-type link
mkagent label create --name "Project" --color "foreground/60"
mkagent label create --name "Alpha" --color "info" --parent-id project
mkagent label create --name "Beta" --color "success" --parent-id project
```
