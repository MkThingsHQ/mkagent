...
MkAgent supports three ways to display tabular data:
...
**Step 1:** Call an approved API or web tool to obtain the JSON response
...
- **No network access:** Scripts should transform data locally after it has been fetched by an approved tool
- **Blocked env vars:** Provider keys, proxy credentials, `AWS_*`, `GITHUB_TOKEN`, `STRIPE_SECRET_KEY`, and `NPM_TOKEN` are removed from the script environment
