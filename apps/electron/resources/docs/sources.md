...
**Before doing anything else**, search for a specialized guide using the mkagent-docs MCP:

```
mcp__mkagent-docs__SearchCraftAgents({ query: "{service} source setup" })
```
...
Agent: [FIRST: Searches for Linear guide]
       mcp__mkagent-docs__SearchCraftAgents({ query: "linear source setup" })
