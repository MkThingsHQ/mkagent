...
This guide explains how to customize the visual theme of MkAgent.

## Overview

MkAgent uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.mkagent/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.mkagent/theme.json` - Override specific colors (app-level)
...
```
~/.mkagent/workspaces/{id}/config.json
```
...
Create `~/.mkagent/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.mkagent/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.mkagent/themes/{name}.json`
...
1. Create `~/.mkagent/theme.json` for overrides or `~/.mkagent/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.mkagent/theme.json` for overrides, `~/.mkagent/themes/` for presets)
