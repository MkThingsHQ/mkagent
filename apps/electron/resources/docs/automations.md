...
This guide explains how to configure automations in MkAgent to automate workflows based on events.

> **CLI-first workflow (recommended):** Use `mkagent automation ...` commands instead of editing JSON directly.
> - `mkagent automation --help`
> - Canonical command reference: [craft-cli.md](./craft-cli.md)

## What Are Automations?

Automations allow you to trigger actions automatically when specific events occur in MkAgent. You can:
...
```
~/.mkagent/workspaces/{workspaceId}/automations.json
```
...
mkagent automation create --event UserPromptSubmit --prompt "..."
mkagent automation update <id> --json '{...}'
...
### App Events (triggered by MkAgent)
...
Send a prompt to MkAgent (creates a new session for scheduled prompts).
...
- Environment variables are expanded (e.g., `$MKAGENT_LABEL`)
...
{
  "type": "webhook",
  "url": "https://hooks.slack.com/services/${MKAGENT_WH_SLACK_PATH}",
  "method": "POST",
  "body": {
    "text": "Session ${MKAGENT_SESSION_NAME} status changed to ${MKAGENT_NEW_STATE}"
  }
}
```
...
```json
{
  ...
  "auth": {
    "type": "bearer",
    "token": "${MKAGENT_WH_API_TOKEN}"
  },
  "body": { "event": "$MKAGENT_EVENT" }
}
```
...
```json
{
  ...
  "auth": {
    "type": "basic",
    "username": "${MKAGENT_WH_USER}",
    "password": "${MKAGENT_WH_PASS}"
  }
}
```
...
**Security:** Webhook actions only have access to `MKAGENT_*` system variables and `MKAGENT_WH_*` user-defined secrets. They do **not** have access to your full system environment (e.g., `$HOME`, `$PATH`, or other process variables).

## Environment Variables

Both prompt and webhook actions support variable expansion using `$VAR` or `${VAR}` syntax.

### System Variables (MKAGENT_*)

These are automatically set by the automation system based on the triggering event:

| Variable | Description | Available For |
|----------|-------------|---------------|
| `$MKAGENT_EVENT` | Event name (e.g., `LabelAdd`) | All events |
| `$MKAGENT_EVENT_DATA` | Full event payload as JSON | All events |
| `$MKAGENT_SESSION_ID` | Session ID | Events with session context |
| `$MKAGENT_SESSION_NAME` | Session name | Events with session context |
| `$MKAGENT_WORKSPACE_ID` | Workspace ID | All events |

**Per-event variables:**

| Event | Variable | Description |
|-------|----------|-------------|
| `LabelAdd` / `LabelRemove` | `$MKAGENT_LABEL` | The label that was added/removed |
| `PermissionModeChange` | `$MKAGENT_OLD_MODE`, `$MKAGENT_NEW_MODE` | Previous and new permission mode |
| `FlagChange` | `$MKAGENT_IS_FLAGGED` | `true` or `false` |
| `SessionStatusChange` | `$MKAGENT_OLD_STATE`, `$MKAGENT_NEW_STATE` | Previous and new status |
| `SchedulerTick` | `$MKAGENT_LOCAL_TIME`, `$MKAGENT_LOCAL_DATE` | Current time (`14:30`) and date (`2026-03-09`) |

### User-Defined Webhook Secrets (MKAGENT_WH_*)

For webhook actions, you can define your own secrets by setting environment variables with the `MKAGENT_WH_` prefix in your shell profile (e.g., `~/.zshrc`, `~/.bashrc`):

```bash
# In your shell profile
export MKAGENT_WH_SLACK_URL="https://hooks.slack.com/services/T.../B.../xxx"
export MKAGENT_WH_DISCORD_URL="https://discord.com/api/webhooks/123/abc"
export MKAGENT_WH_API_TOKEN="your-secret-token"
```
...
```json
{
  "type": "webhook",
  "url": "https://api.example.com/events",
  "headers": { "Authorization": "Bearer ${MKAGENT_WH_API_TOKEN}" },
  "body": { "event": "${MKAGENT_EVENT}", "session": "${MKAGENT_SESSION_NAME}" }
}
```

This keeps secrets out of `automations.json` (which may be shared or committed to version control).

> **Note:** Only variables prefixed with `MKAGENT_WH_` are injected into webhook actions. Other environment variables (like `$HOME` or `$DATABASE_URL`) are not accessible to webhooks.
...
4. **Pair the supergroup.** In MkAgent: Settings → Messaging → Telegram → **Pair Supergroup**. Copy the 6-digit code, then in any topic of the supergroup type `/pair <code>`. The bot confirms and the Settings row updates with the group's title.

Verify by checking the supergroup row in Settings shows the group title. If automation runs fail later, `~/.mkagent/logs/messaging-gateway.log` will show `automation_topic_bind_failed` with the underlying Telegram error.
        ...
        "actions": [
          {
            ...
            "url": "${MKAGENT_WH_SLACK_URL}",
            "method": "POST",
            "body": { "text": ":warning: Permission escalated from safe to allow-all in *${MKAGENT_SESSION_NAME}*" }
          }
        ]
      }
    ]
...
```json
{
  ...
  "automations": {
    "LabelAdd": [
      {
        "actions": [
          { "type": "prompt", "prompt": "The label $MKAGENT_LABEL was added. Log this change with a timestamp." }
        ]
      }
    ],
    "LabelRemove": [
      {
        "actions": [
          { "type": "prompt", "prompt": "The label $MKAGENT_LABEL was removed. Log this change with a timestamp." }
        ]
      }
    ]
  }
...
Sends a Slack message when a session is marked as done. Requires `MKAGENT_WH_SLACK_URL` in your shell profile.
        ...
        "actions": [
          {
            ...
            "method": "POST",
            "body": {
              "text": ":white_check_mark: Session *${MKAGENT_SESSION_NAME}* marked as done"
            }
          }
        ]
      }
        ...
        "actions": [
          {
            ...
            "url": "${MKAGENT_WH_SLACK_URL}",
            "method": "POST",
            "body": { "text": ":rotating_light: Urgent label added to *${MKAGENT_SESSION_NAME}*" }
          },
          {
            "type": "prompt",
        ...
        "actions": [
          {
            ...
            "body": {
              ...
              "client_id": "${MKAGENT_WH_CLIENT_ID}",
              "client_secret": "${MKAGENT_WH_CLIENT_SECRET}"
            }
          }
        ]
      }
        ...
        "actions": [
          {
            ...
            "method": "POST",
            "headers": {
              "Authorization": "Bearer ${MKAGENT_WH_API_TOKEN}",
              "X-Source": "mkagent"
            },
            "body": {
              "event": "${MKAGENT_EVENT}",
              "session_id": "${MKAGENT_SESSION_ID}",
              "old_status": "${MKAGENT_OLD_STATE}",
              "new_status": "${MKAGENT_NEW_STATE}"
            }
          }
        ]
      }
...
Ask MkAgent to validate your automations configuration:
...
2. **Check env vars** — Ensure `MKAGENT_WH_*` variables are set in your shell profile and MkAgent was restarted after adding them. URLs using `$VAR` templates are validated after variable expansion — if the variable is empty or unset, the URL will be invalid.
...
5. **Keep secrets out of config** - Use `MKAGENT_WH_*` env vars for webhook URLs and tokens instead of hardcoding them in automations.json
