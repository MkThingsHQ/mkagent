#!/usr/bin/env bun
/**
 * Build script for standalone MkAgent server.
 *
...
function showHelp(): void {
  console.log(`
Standalone server build script for MkAgent
 ...
 * not node: builtins, not workspace @mkagent/* packages).
 */
    ...
    for (const entry of readdirSync(d, { withFileTypes: true })) {
      ...
      if (entry.isDirectory() && entry.name !== 'node_modules' && entry.name !== 'tests' && entry.name !== '__tests__') {
        ...
        while ((match = importRe.exec(content)) !== null) {
          ...
          if (spec.startsWith('.') || spec.startsWith('node:') || spec.startsWith('@mkagent/')) continue;
...
function copyWorkspacePackages(config: ServerBuildConfig): void {
  ...
  // MKAGENT_MESSAGING_WA_WORKER env resolution in packages/server/src/index.ts.
  const packages = [
    'server',
    'server-core',
    'shared',
...
function createRootConfig(config: ServerBuildConfig): void {
  ...
  // Root package.json with workspaces (Bun resolves @mkagent/* through this)
  const rootPkg = {
    name: 'mkagent-server-dist',
    version,
    private: true,
    workspaces: ['packages/*'],
  };
  ...
  const rootTsconfig = {
    compilerOptions: {
      ...
      paths: {
        '@mkagent/server-core/*': ['./packages/server-core/src/*'],
        '@mkagent/shared/*': ['./packages/shared/src/*'],
        '@mkagent/core/*': ['./packages/core/src/*'],
        '@mkagent/session-tools-core/*': ['./packages/session-tools-core/src/*'],
      },
    },
  };
  writeFileSync(join(outputDir, 'tsconfig.json'), JSON.stringify(rootTsconfig, null, 2) + '\n');

  // Create workspace symlinks in node_modules/@mkagent/
  // Bun needs these to resolve workspace package imports at runtime
  const scopeDir = join(outputDir, 'node_modules', '@mkagent');
  ...
  if (existsSync(packagesDir)) {
    for (const pkg of readdirSync(packagesDir)) {
      ...
      try {
        ...
        if (name.startsWith('@mkagent/')) {
          const shortName = name.replace('@mkagent/', '');
...
function createEntryScripts(config: ServerBuildConfig): void {
  ...
  // bin/mkagent-server — main entry wrapper
...
export MKAGENT_UV="$ROOT/resources/bin/uv"
...
`;
  writeFileSync(join(binDir, 'mkagent-server'), craftServer);

  // start.sh — convenience entry
  const startSh = `#!/bin/sh
# MkAgent Server — convenience entry point
DIR="$(cd "$(dirname "$0")" && pwd)"
exec "$DIR/bin/mkagent-server" "$@"
`;
...
echo "=== MkAgent Server Setup ==="
echo ""

# Make binaries executable
chmod +x "$DIR/bin/mkagent-server" "$DIR/start.sh"
...
if [ -z "\${MKAGENT_SERVER_TOKEN:-}" ]; then
  TOKEN=\$(openssl rand -hex 32)
  cat > "$DIR/.env" <<ENVFILE
MKAGENT_SERVER_TOKEN=$TOKEN

# TLS — uncomment and set paths to enable wss://
# MKAGENT_RPC_TLS_CERT=/path/to/cert.pem
# MKAGENT_RPC_TLS_KEY=/path/to/key.pem
# MKAGENT_RPC_TLS_CA=/path/to/ca.pem
ENVFILE
  echo ""
  echo "Generated server token (saved to $DIR/.env)"
else
  TOKEN="\$MKAGENT_SERVER_TOKEN"
  echo ""
  echo "Using MKAGENT_SERVER_TOKEN from environment."
fi
...
if [ "\${1:-}" = "--systemd" ]; then
  ...
  SERVICE_USER="\${MKAGENT_USER:-\$(logname 2>/dev/null || echo craft)}"
  SERVICE_FILE="/etc/systemd/system/mkagent-server.service"

  cat > "$SERVICE_FILE" <<UNIT
[Unit]
Description=MkAgent Server
...
Environment=MKAGENT_RPC_HOST=127.0.0.1
Environment=MKAGENT_RPC_PORT=9100
ExecStart=$DIR/bin/mkagent-server
Restart=on-failure
RestartSec=5
...
UNIT
  ...
  systemctl enable mkagent-server

  echo ""
  echo "Systemd service installed."
  echo "  Start:   sudo systemctl start mkagent-server"
  echo "  Status:  sudo systemctl status mkagent-server"
  echo "  Logs:    journalctl -u mkagent-server -f"
  echo ""
  exit 0
fi
...
echo ""
echo "Quick start:"
echo "  MKAGENT_SERVER_TOKEN=$TOKEN $DIR/start.sh"
echo ""
...
# Make binaries executable
RUN chmod +x bin/mkagent-server vendor/bun/bun resources/bin/uv && \\
    for f in resources/bin/*; do [ -f "$f" ] && chmod +x "$f"; done

ENV MKAGENT_IS_PACKAGED=true
ENV MKAGENT_BUNDLED_ASSETS_ROOT=/app
ENV MKAGENT_APP_ROOT=/app
ENV MKAGENT_RESOURCES_PATH=/app/resources
ENV MKAGENT_UV=/app/resources/bin/uv
ENV MKAGENT_SCRIPTS=/app/resources/scripts
ENV MKAGENT_RPC_HOST=0.0.0.0
ENV MKAGENT_RPC_PORT=9100
ENV PATH="/app/resources/bin:/app/vendor/bun:\${PATH}"

EXPOSE 9100

ENTRYPOINT ["/app/bin/mkagent-server"]
`;
...
services:
  mkagent-server:
    ...
    ports:
      - "9100:9100"
    environment:
      - MKAGENT_SERVER_TOKEN=\${MKAGENT_SERVER_TOKEN:?Set MKAGENT_SERVER_TOKEN}
      - MKAGENT_RPC_PORT=9100
      # TLS — uncomment to enable wss://
      # - MKAGENT_RPC_TLS_CERT=/certs/cert.pem
      # - MKAGENT_RPC_TLS_KEY=/certs/key.pem
    volumes:
      - craft-data:/root/.mkagent
...
async function main(): Promise<void> {
  ...
  console.log(`=== Building MkAgent Server ${version} for ${platform}-${arch} ===`);
  ...
  if (config.compress) {
    const archiveName = `mkagent-server-${version}-${platform}-${arch}.tar.gz`;
  ...
  console.log(`\nQuick start:`);
  console.log(`  MKAGENT_SERVER_TOKEN=<secret> ${outputDir}/start.sh`);
}
