...
function browserDocPath(): string {
  return resolve(join(homedir(), '.mkagent', 'docs', 'browser-tools.md'));
}
...
describe('PrerequisiteManager', () => {
  ...
  describe('rule matching', () => {
    ...
    it('exempts mkagent-docs MCP tools', () => {
      mockExistsPaths.add(guidePath('mkagent-docs'));
      const result = manager.checkPrerequisites('mcp__mkagent-docs__search');
      expect(result.allowed).toBe(true);
    });
