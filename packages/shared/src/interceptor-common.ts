 ...
 *
 * The interceptor runs as a preload script in the Pi subprocess.
...
export const DEBUG = INTERCEPTOR_LOGGING_ENABLED &&
  (process.argv.includes('--debug') || process.env.MKAGENT_DEBUG === '1');

/** Config file path for reading settings in the SDK subprocess */
export const CONFIG_FILE = join(homedir(), '.mkagent', 'config.json');

/** Session directory — set by env var (subprocess) or setSessionDir() (main process) */
let _sessionDir: string | null = process.env.MKAGENT_SESSION_DIR || null;

// ============================================================================
// LOGGING
// ============================================================================

export const LOG_DIR = join(homedir(), '.mkagent', 'logs');
...
function getErrorFilePath(): string {
  ...
  return join(homedir(), '.mkagent', 'api-error.json');
}
 ...
 * - SDK subprocess: MKAGENT_SESSION_DIR env var (set by main process before spawn)
