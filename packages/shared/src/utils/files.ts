...
export function atomicWriteFileSync(filePath: string, data: string): void {
  // A unique name prevents concurrent writers from clobbering the same temp
  // file before either write can be atomically renamed into place.
...
function readClipboardMacOS(): FileAttachment[] {
  ...
  try {
    const scriptFile = join(tmpdir(), `mkagent-clipboard-files-${Date.now()}.js`);
    const jxaScript = `
ObjC.import('AppKit');
...
function readClipboardImageDataWindows(): FileAttachment | null {
  const tempFile = join(tmpdir(), `mkagent-clipboard-${Date.now()}.png`);
...
function readClipboardImageDataMacOS(): FileAttachment | null {
  ...
  try {
    const scriptFile = join(tmpdir(), `mkagent-clipboard-script-${Date.now()}.js`);
    const jxaScript = `
ObjC.import('AppKit');
ObjC.import('Foundation');
