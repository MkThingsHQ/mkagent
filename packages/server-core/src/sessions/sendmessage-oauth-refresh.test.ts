...
describe('sendMessage OAuth refresh ordering (#710)', () => {
  ...
  function writeSkill(slug: string, requiredSources: string[]) {
    const dir = join(tmpRoot, 'skills', slug)
    mkdirSync(dir, { recursive: true })
    writeFileSync(
      join(dir, 'SKILL.md'),
      `---\nname: ${slug}\ndescription: Source-aware skill\nrequiredSources:\n${requiredSources.map(source => `  - ${source}`).join('\n')}\n---\n\n# ${slug}\n`,
    )
  }
  ...
  it('pre-enables usable sources required by invoked skills', async () => {
    const sessionId = 'skill-required-source'
    const managed = buildSession(sessionId)
    writeOAuthSource('skill-source', true)
    writeSkill('source-skill', ['skill-source'])

    await sm.sendMessage(sessionId, 'use the skill', undefined, undefined, {
      skillSlugs: ['source-skill'],
    }).catch(() => { /* expected: no LLM platform in this harness */ })

    expect(managed.enabledSourceSlugs).toEqual(['skill-source'])
  })
})
