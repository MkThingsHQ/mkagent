 ...
 * Replaces provider-specific model refresh functions.
 *
...
class ModelRefreshService {
  ...
  private async _doRefresh(slug: string): Promise<void> {
    ...
    try {
      ...
      handlerLog.info(`Model refresh [${slug}]: fetching (provider=${connection.providerType}, piAuth=${connection.piAuthProvider})`)
