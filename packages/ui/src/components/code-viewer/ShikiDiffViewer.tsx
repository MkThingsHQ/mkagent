...
import { registerMkAgentShikiThemes } from './registerShikiThemes'
...
export interface ShikiDiffViewerProps {
   ...
   *  Shiki theme natively. Falls back to mkagent-dark/mkagent-light (transparent bg) if not set. */
  shikiTheme?: string
...
export function ShikiDiffViewer({
  ...
  // to mkagent-dark/mkagent-light which have transparent bg for CSS variable theming
  const resolvedThemeName = shikiTheme || (theme === 'dark' ? 'mkagent-dark' : 'mkagent-light')
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      ;(header as any).__mkagentClickCleanup = () => header.removeEventListener('click', handleClick)
    }, 150)
    ...
    return () => {
      ...
      const header = diffsContainer?.shadowRoot?.querySelector('[data-diffs-header]')
      if (header) {
        ;(header as any).__mkagentClickCleanup?.()
      }
    }
